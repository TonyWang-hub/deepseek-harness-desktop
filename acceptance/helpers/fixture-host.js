...
async function respondApproval(baseUrl, envelope) {
...
async function openEventSocket(baseUrl) {
...
async function replayScenario(baseUrl, workspace) {
  const socket = await openEventSocket(baseUrl)
  ...
  socket.addEventListener('message', event => {
    ...
    const response = respondApproval(baseUrl, envelope)
  ...
  try {
    const createdWorkspace = await rpc(baseUrl, 'workspace.create', { path: workspace })
    ...
    const history = await waitUntil(async () => {
      if (socketFailure) throw socketFailure
      const candidate = await rpc(baseUrl, 'session.history', { sessionId })
      return candidate.events.some(entry => entry.event.type === 'turn/end') ? candidate : undefined
    }, 'parity turn completion', 60_000)
    await Promise.all(pendingResponses)
    if (socketFailure) throw socketFailure
    return normalizeParityHistory(history.events, { workspace })
  } finally {
    socket.close()
  }
}
...
export async function runParityEntry(entry) {
  ...
  try {
    ...
    baseUrl = new URL(application.page.url()).origin
    return await replayScenario(baseUrl, workspace)
  } finally {
