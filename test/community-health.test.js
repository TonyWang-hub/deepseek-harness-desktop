...
test('community guidance is bilingual and routes security reports privately', async () => {
  ...
  assert.match(readme, /v0\.4\.1 is the latest signed and notarized macOS release/)
  assert.match(readmeChinese, /v0\.4\.1 是当前最新的已签名、公证 macOS 正式版本/)
  ...
  assert.match(support, /v0\.4\.0 or earlier.*v0\.4\.1 DMG/s)
  assert.match(support, /v0\.4\.0 或更早版本.*v0\.4\.1 DMG/s)
...
test('issue forms and the pull request template collect actionable evidence', async () => {
  ...
  assert.match(bugSource, /placeholder: 0\.4\.1 or commit SHA/)
