...
test('community guidance is bilingual and routes security reports privately', async () => {
  ...
  assert.match(readme, /v0\.4\.4 is the latest signed and notarized macOS release/)
  assert.match(readmeChinese, /v0\.4\.4 是当前最新的已签名、公证 macOS 正式版本/)
  ...
  assert.match(readme, /Proof status: in progress/)
  assert.match(readmeChinese, /证明状态：进行中/)
  ...
  assert.match(support, /v0\.4\.2 or earlier.*v0\.4\.4 DMG/s)
  assert.match(support, /v0\.4\.2 或更早版本.*v0\.4\.4 DMG/s)
...
test('issue forms and the pull request template collect actionable evidence', async () => {
  ...
  assert.match(bugSource, /placeholder: 0\.4\.4 or commit SHA/)
