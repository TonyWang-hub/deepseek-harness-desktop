...
| FR-004 | Must | The desktop-owned connection shall carry all official resource, API, RPC, streaming, reconnect, and cancellation traffic through a generic versioned transport/bootstrap adapter rather than one desktop method per product capability. The adapter may reproduce browser transport primitives but shall not own DSH state, interaction semantics, or business UI. | Adding or replaying an inventory capability requires no new shell-specific business channel, and order/cancellation tests pass. |
...
| FR-010 | Must | The project shall maintain a generated parity inventory covering every first-party browser contribution plus representative cross-cutting user flows. Its expected set shall be derived independently from the pinned payload closure and official profile, then compared with the live boot graph. | Inventory generation is reproducible, a deliberately removed or corrupted contribution makes the oracle fail, and every item maps to an automated or recorded acceptance check. |
...
| FR-013 | Must | Benchmark tooling shall capture attributable whole-application process membership, macOS physical footprint, diagnostic RSS, responsiveness, operational startup phases, event integrity, and environment identity for each sample. | Raw per-sample records identify all attributable shell, WebView, GPU/network/content, Host, and descendant processes and can be aggregated without manual reconstruction. |
...
| NFR-002 | Must | During sustained reference streams, input-to-next-paint latency shall be at most 50 ms at p95 and below 100 ms at p99. | At least 30 randomized runs per declared stream class on the reference Mac, with per-interaction latency samples and confidence intervals reported. |
...
| NFR-004 | Must | Warm operational-prompt-ready launch shall complete within 1.5 seconds at p95 and cold operational-prompt-ready launch within 3 seconds at p95 on the reference Mac. Operational prompt-ready requires a visible, responsive composer connected to a ready Host and able to start the first session. | At least 30 randomized warm and 30 randomized cold launches with process start, window visible, renderer settled, Host connected, composer operational, and first-session-ready phases reported separately. |
...
| NFR-006 | Must | The desktop transport and shell shall add only bounded retained state over the official browser baseline and show no superlinear desktop-added growth across the 100,000-event fixture. Destroying a renderer shall release the retired renderer and proxy state after quiescence; session-level retention remains upstream-owned. | Candidate-minus-browser retained buffers and attributable memory slopes remain within predeclared ceilings, and renderer recreation demonstrates reclamation without requiring an upstream session-policy change. |
...
| NFR-008 | Must | Tauri shall replace the Electron compatibility baseline only if its predeclared aggregate p95 attributable physical footprint is at least 25% lower across representative idle, long-history, sustained-stream, and post-switch scenarios; no individual scenario may regress by more than 5%, and responsiveness or launch may neither miss a hard budget nor regress by more than 10%. | The deterministic comparison report applies the aggregate threshold, per-scenario guardrail, and response/startup guardrails to matched valid runs. |
...
| C-003 | Fixed | Upstream DSH is consumed as an exact versioned dependency or read-only local checkout. Forks, vendored source edits, DSH package monkey-patches, and patch-package changes are prohibited. The generic pre-entry browser primitive provider authorized by C-010 is an explicit transport adapter, not a DSH code mutation. |
...
| C-010 | Fixed | A desktop transport/bootstrap adapter is permitted only to reproduce generic browser resource, Fetch, WebSocket, cancellation, and lifecycle semantics. It shall not duplicate DSH business methods, session projections, interaction behavior, or product UI. |
...
| SC-004 | The 100,000-event and sustained-stream scenarios preserve final-state equality, keep desktop-added buffering bounded relative to the browser baseline, reclaim retired renderer state, and meet the p95/p99 interaction, control-latency, and stall thresholds. |
...
| SC-008 | A new contributor can clone the standalone repository, run the deterministic foundation checks, and launch both required development vertical slices using documented commands without editing upstream source. If neither candidate passes selection gates, the evaluator records no-selection and produces a remediation input instead of naming a production shell. |
