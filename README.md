...
> **Distribution status — v0.4.2 is the latest signed and notarized macOS release.** Download only from this repository's [latest release](https://github.com/TonyWang-hub/deepseek-harness-desktop/releases/latest), choose the native architecture, and verify the published checksum and Apple signature. Locally generated unsigned candidates remain test artifacts and must not be redistributed as official releases.
>
> **Upgrade warning — the strict public updater proof failed:** Signed v0.4.1 discovered v0.4.2, accepted the verified public ZIP, and transferred it into the native ShipIt cache, but native install authorization did not arrive within five minutes; safe fallback left v0.4.1 installed. Do not rely on automatic installation from v0.4.1 or earlier. Install the matching v0.4.2 DMG manually; the fix will use a new release (v0.4.3 at the earliest), without moving or rebuilding the v0.4.2 tag or assets.
...
The [latest release, v0.4.2](https://github.com/TonyWang-hub/deepseek-harness-desktop/releases/tag/v0.4.2), provides signed, notarized, and stapled macOS arm64/x64 artifacts, checksums, and update metadata. Its public installed-update proof failed, so use the matching DMG rather than relying on automatic replacement. Repositories with similar names publish independent community builds with different code, data paths, update policies, and signing status.
...
v0.4.1 introduced an explicit updater install path after exact Host shutdown and keeps ordinary quits intercepted while waiting for Electron's native Squirrel authorization. The real signed v0.4.1→public v0.4.2 proof nevertheless timed out after Squirrel had accepted and transferred the verified update: `before-quit-for-update` never arrived, safe fallback exited, and v0.4.1 remained installed. Install the v0.4.2 DMG manually when upgrading from v0.4.1 or earlier; automatic installation is not claimed as working. Source builds and smoke mode do not check for updates.
...
| --- | --- | --- | --- | --- |
| This project | [v0.4.2](https://github.com/TonyWang-hub/deepseek-harness-desktop/releases/tag/v0.4.2): macOS arm64/x64 DMG and ZIP, update metadata, and checksums | Pinned, unmodified npm payload; standard `$DSH_HOME`; bundled runtime; wake recovery and private diagnostics | Public dual-architecture feed; v0.4.1→v0.4.2 automatic install proof failed, so manual DMG install is required | Release workflow verifies `codesign`, Gatekeeper, a stapled notarization ticket, packaged acceptance, and mounted DMGs |
